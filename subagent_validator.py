...
class DataValidator:
    ...
    def _compare_key_findings(self, existing: str, new: str) -> dict:
        ...
        if new_len > existing_len * 1.5:  # New is 50% longer
            return {
                "field": "key_findings",
                "existing_value": existing[:50] + "..." if existing_len > 50 else existing,
    ...
    def _compare_data_quality(self, existing: str, new: str) -> dict:
        """比较数据质量等级"""
        quality_rank = {"verified": 3, "partial": 2, "pending_verification": 1, "pending": 1}
    ...
    def validate(self, compound_name: str, new_data: dict) -> dict:
        ...
        if existing_findings or new_findings:
            conflicts.append(self._compare_key_findings(existing_findings, new_findings))
        ...
        if final_decision == "merge":
            ...
            if any(c['field'] == 'clinical_trials' for c in conflicts):
                trial_conflict = next(c for c in conflicts if c['field'] == 'clinical_trials')
            ...
            if any(c['field'] == 'chemical_identifiers.pubchem_cid' and c['recommendation'] == 'keep_existing' for c in conflicts):
                merged_data['chemical_identifiers'] = existing_chem
            
            # Accept new key findings if better
            if any(c['field'] == 'key_findings' and c['recommendation'] == 'accept_new' for c in conflicts):
...
def validate_nn9487_test():
    ...
    new_data = {
        ...
        "patents": [],
        "pubmed_papers": [
            {"pmid": "40550231", "title": "Amycretin, a novel, unimolecular GLP-1 and amylin receptor agonist...", "year": "2025"},
            {"pmid": "40550229", "title": "Safety, tolerability, pharmacokinetics, and pharmacodynamics...", "year": "2025"},
            {"pmid": "40706446", "title": "The effect of amycretin... on body weight...", "year": "2025"},
            {"pmid": "41054801", "title": "Novel GLP-1-based Medications...", "year": "2026"},
            {"pmid": "41850421", "title": "Amycretin in obesity: Mechanisms, clinical efficacy...", "year": "2026"}
        ],
        "suppliers": [

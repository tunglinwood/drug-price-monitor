...
# 导入简单认证模块
from simple_auth import require_login, show_user_info
...
if "data_loaded_at" in st.session_state:
    st.info(
        f"📊 **Data Loaded:** {st.session_state['data_loaded_at']} | **Git Commit:** `{st.session_state.get('git_commit', 'Unknown')}`"
    )
...
def load_data(compounds_file="compounds.json"):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            st.session_state["data_loaded_at"] = datetime.now().strftime(
                "%Y-%m-%d %H:%M:%S"
            )
            st.session_state["git_commit"] = data.get("metadata", {}).get(
                "git_commit", "Unknown"
            )
            return data
        else:
...
if dashboard is None:
    if board == "THR-Beta Compounds":
        st.warning(
            f"⚠️ {compounds_file} not found yet. Please provide THR-beta compound list to create this file."
        )
        st.info("💡 Switch to GLP-1 Compounds board to view GLP-1 data.")
    elif board == "FGF21 Analogues":
        st.warning(
            f"⚠️ {compounds_file} not found yet. Please provide FGF21 analogue compound list to create this file."
        )
        st.info(
            "💡 Switch to GLP-1 Compounds or THR-Beta Compounds board to view existing data."
        )
    elif board == "Pan-PPAR Agonists":
        st.warning(
            f"⚠️ {compounds_file} not found yet. Please provide Pan-PPAR agonist compound list to create this file."
        )
        st.info(
...
with col2:
    data_quality_filter = st.selectbox(
        "数据质量", ["全部", "verified", "partial", "pending_verification"]
    )
...
if search_term:
    filtered = [
        c
        for c in filtered
        if search_term.lower() in c.get("chem_name", "").lower()
...
if stage_filter != "全部":
    filtered = [
        c for c in filtered if stage_filter.lower() in c.get("stage", "").lower()
    ]
...
if not filtered:
    ...
    for comp in filtered:
        ...
        if quality == "verified":
            quality_badge = (
                '<span class="data-quality-badge badge-verified">✅ VERIFIED</span>'
            )
        elif quality == "partial":
            quality_badge = (
                '<span class="data-quality-badge badge-partial">⚠️ PARTIAL</span>'
            )
        else:
            quality_badge = (
                '<span class="data-quality-badge badge-pending">⏳ PENDING</span>'
            )
        ...
        # 关键发现（截断）
        key_findings = (
            comp.get("key_findings", "")[:80] + "..."
            if len(comp.get("key_findings", "")) > 80
            else comp.get("key_findings", "")
        )
    ...
    if selected:
        ...
        with col1:
            ...
            if chem.get("pubchem_cid"):
                st.markdown(
                    f"**PubChem CID:** [{chem.get('pubchem_cid')}](https://pubchem.ncbi.nlm.nih.gov/compound/{chem.get('pubchem_cid')})"
                )
        ...
        with col2:
            ...
            if papers:
                ...
                for paper in papers:
                    ...
                    if pmid:
                        st.markdown(
                            f"- [{title}](https://pubmed.ncbi.nlm.nih.gov/{pmid}/) ({year})"
                        )
                    else:
